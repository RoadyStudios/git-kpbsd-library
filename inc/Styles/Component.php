...
use function WP_Rig\WP_Rig\kpbsdlibrary;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'kpbsdlibrary-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = kpbsdlibrary()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'kpbsdlibrary-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'kpbsdlibrary' ), $handle ) ), 'KPBSD Library 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'kpbsdlibrary_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! kpbsdlibrary()->is_amp();
		 ...
		 */
		return apply_filters( 'kpbsdlibrary_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			'kpbsdlibrary-global'     => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			'kpbsdlibrary-comments'   => array(
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			),
			'kpbsdlibrary-content'    => array(
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			),
			'kpbsdlibrary-sidebar'    => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return kpbsdlibrary()->is_primary_sidebar_active();
				},
			),
			'kpbsdlibrary-widgets'    => array(
				'file'             => 'widgets.min.css',
				'preload_callback' => function() {
					return kpbsdlibrary()->is_primary_sidebar_active();
				},
			),
			'kpbsdlibrary-front-page' => array(
		 ...
		 */
		$css_files = apply_filters( 'kpbsdlibrary_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = array(
			...
			'Roboto'     => array( '400', '400i', '600', '600i' ),
		);
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'kpbsdlibrary_google_fonts', $google_fonts );
