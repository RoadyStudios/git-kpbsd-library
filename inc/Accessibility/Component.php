...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'kpbsdlibrary-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			kpbsdlibrary()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'kpbsdlibrary-navigation', 'async', true );
		wp_script_add_data( 'kpbsdlibrary-navigation', 'precache', true );
		wp_localize_script(
			'kpbsdlibrary-navigation',
			'kpbsdlibraryScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'kpbsdlibrary' ),
				'collapse' => __( 'Collapse child menu', 'kpbsdlibrary' ),
			)
		);
	}
